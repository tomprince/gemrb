...
public:
	...
	void ReadWord(ieWord* dest);
	void ReadWordSigned (ieWordSigned* dest);
	void ReadDword(ieDword* dest);
	void ReadResRef(ieResRef dest);
	virtual int Write(const void* src, unsigned int len) = 0;
	void WriteWord(const ieWord* src);
	void WriteDword(const ieDword* src);
	void WriteResRef(const ieResRef src);
