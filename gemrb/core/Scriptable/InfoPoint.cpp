...
void InfoPoint::Dump() const
{
	...
	switch (Type) {
		case ST_TRIGGER:
			buffer.format( "Debugdump of InfoPoint Region %s:\n", GetScriptName() );
			break;
		case ST_PROXIMITY:
			buffer.format( "Debugdump of Trap Region %s:\n", GetScriptName() );
			break;
		case ST_TRAVEL:
			buffer.format( "Debugdump of Travel Region %s:\n", GetScriptName() );
			break;
		default:
			buffer.format( "Debugdump of Unsupported Region %s:\n", GetScriptName() );
			break;
	}
	buffer.format( "Region Global ID: %d\n", GetGlobalID());
	buffer.format( "Position: %d.%d\n", Pos.x, Pos.y);
	switch(Type) {
	case ST_TRAVEL:
		buffer.format( "Destination Area: %s Entrance: %s\n", Destination, EntranceName);
		break;
	case ST_PROXIMITY:
		buffer.format( "TrapDetected: %d, Trapped: %s\n", TrapDetected, YESNO(Trapped));
		buffer.format( "Trap detection: %d%%, Trap removal: %d%%\n", TrapDetectionDiff,
			TrapRemovalDiff );
		break;
	case ST_TRIGGER:
		buffer.format ( "InfoString: %s\n", overHeadText );
		break;
	default:;
	}
	...
	buffer.format( "Script: %s, Key: %s, Dialog: %s\n", name, KeyResRef, Dialog );
	buffer.format( "Deactivated: %s\n", YESNO(Flags&TRAP_DEACTIVATED));
	buffer.format( "Active: %s\n", YESNO(InternalFlags&IF_ACTIVE));
