...
void InitializeIEScript()
{
	...
	while (j--) {
		...
		if (triggers[i]) {
			if (poi && triggers[i]!=poi->Function) {
				StringBuffer buffer;
				buffer.format("%s is in collision with ",
				...
				if (InDebug&ID_TRIGGERS) {
					StringBuffer buffer;
					buffer.format("%s is a synonym of ",
						triggersTable->GetStringIndex( j ) );
					printFunction(buffer, triggersTable, triggersTable->FindValue(triggersTable->GetValueIndex(j)));
	...
	if (overrideActionsTable) {
		...
		while (j--) {
			...
			if (actions[i]) {
				StringBuffer buffer;
				buffer.format("%s overrides existing action ",
	...
	if (overrideTriggersTable) {
		...
		while (j--) {
			...
			if (triggers[i]) {
				StringBuffer buffer;
				buffer.format("%s overrides existing trigger ",
